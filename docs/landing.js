// JavaScript Controller for Quantum Strategy Lab Landing Page
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const configData = {
    "1d_base": {
      stats: [51.3, 43.1, 57.8, 51.8, 49.2, 30], // Accuracy, F1, Return, RiskControl, Sharpe, CostEfficiency
      statsFormatted: ["51.3%", "43.1%", "57.8%", "普通 (-48.2%)", "0.82", "偏低 (高換手)"],
      color: "#0ea5e9", // blue
      bg: "#070c15"
    },
    "1d_vol": {
      stats: [51.9, 45.6, 54.6, 53.5, 51.0, 32],
      statsFormatted: ["51.9%", "45.6%", "54.6%", "中等 (-46.5%)", "0.85", "偏低 (高換手)"],
      color: "#a855f7", // purple
      bg: "#0c0814"
    },
    "5d_base": {
      stats: [54.1, 62.1, 43.1, 74.7, 67.2, 78],
      statsFormatted: ["54.1%", "62.1%", "43.1%", "良好 (-25.3%)", "1.12", "良好 (低換手)"],
      color: "#14b8a6", // teal
      bg: "#050f12"
    },
    "5d_vol": {
      stats: [52.9, 57.4, 56.0, 77.9, 79.2, 82],
      statsFormatted: ["52.9%", "57.4%", "56.0%", "卓越 (-22.1%)", "1.32", "極佳 (低換手)"],
      color: "#f97316", // orange
      bg: "#120a06"
    }
  };
  ...
  const maxVal = 100;
  const maxRadius = 180;
  // Angles for 6 dimensions: Accuracy, F1 Score, Strategy Return, Risk Control, Sharpe Ratio, Cost Efficiency
  const angles = [
    -Math.PI / 2,         // 12 o'clock (Accuracy)
    -Math.PI / 6,         // 2 o'clock (F1 Score)
    Math.PI / 6,          // 4 o'clock (Return)
    Math.PI / 2,          // 6 o'clock (Risk Control)
    Math.PI * 5 / 6,      // 8 o'clock (Sharpe Ratio)
    Math.PI * 7 / 6       // 10 o'clock (Cost Efficiency)
  ];
  ...
  function endLoader() {
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        updateStatsMatrix("1d_base", false);
  ...
  // Hero Candlestick Chart Fly-Away
  ...
  // ==========================================
  // 9. STATS MATRIX MORPHING ENGINE
  // ==========================================
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      if (!radarInitiallyDrawn) {
        updateStatsMatrix("1d_base", true);
  ...
  function updateStatsMatrix(configKey, animate = true) {
    ...
    if (animate) {
      ...
      gsap.to("#radar-poly", {
        ...
        fill: `${data.color}20`, // Hex 12% opacity
        stroke: data.color,
        duration: 0.8,
        ease: "power2.out"
      });
      ...
      gsap.to(".stats-bg-glow", {
        background: `radial-gradient(circle, ${data.color}2d 0%, rgba(7, 9, 14, 0) 70%)`,
        duration: 1.0
      });
      ...
      document.getElementById("radar-poly").style.fill = `${data.color}20`;
  ...
  // Register click events for Stats selectors
